...
//
// DynamoDB の ProvisionedThroughputExceededException や一時的なサーバエラーに対して、
// AWS 公式推奨の「指数バックオフ + フルジッター」方式で自動リトライする。
//
// Python 版・TypeScript 版と同じ設計思想の並置実装。
//
// 設計方針:
//   - Sleep / Rand を差し替え可能にして、テストを決定的かつ実待機ゼロに保つ
//   - リトライ不能なエラー（ValidationException / AccessDenied 等）は即座に返す
